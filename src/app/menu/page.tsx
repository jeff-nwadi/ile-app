  ...
  return (
    <>
      ...
      <main className="pt-32 px-[6vw] pb-32 max-w-5xl mx-auto">
        ...
        <h1 className="font-serif mt-3 mb-12" style={{ fontSize: "clamp(34px,5vw,56px)" }}>
          Tonight&apos;s Menu
        </h1>
                ...
                {catItems.map((item) => (
                  <div key={item.id} className="flex items-center justify-between gap-6 py-5">
                    <div>
                      ...
                      <p className="text-sm opacity-60 mt-1 max-w-md">{item.description}</p>
                      <p className="font-mono text-sm text-indigo mt-1">
                        {formatNaira(item.priceKobo)}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 shrink-0">
                      <Button size="icon" variant="outline" onClick={() => addToCart(item.id, -1)}>
                        −
                      </Button>
                      <span className="w-6 text-center font-mono">{cart[item.id] ?? 0}</span>
      ...
      {cartItems.length > 0 && !showCheckout && (
        <div className="fixed bottom-0 left-0 right-0 bg-charcoal text-ivory px-[6vw] py-5 flex items-center justify-between z-40">
          <span className="font-mono text-sm">
            {cartItems.reduce((n, c) => n + c.qty, 0)} items — {formatNaira(total)}
          </span>
      ...
      {showCheckout && (
        <div className="fixed inset-0 bg-charcoal/70 z-50 flex items-center justify-center p-6">
          ...
          >
            ...
            <p className="font-mono text-xs opacity-60">Total: {formatNaira(total)}</p>
